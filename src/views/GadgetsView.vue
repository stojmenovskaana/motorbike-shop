    ...
    <main class="product-page">
      ...
      <div class="content">
        ...
        <section>
          ...
          >
            <article
              v-for="gadget in paginatedProducts"
              :key="gadget.id"
              class="product-card"
              @click="goToProduct(gadget.id)"
            >
              <img
                :src="gadget.image"
                :alt="gadget.name"
              ...
              <div class="product-actions">
                <button
                  class="favorite"
                  :class="{ active: isFavorite(gadget) }"
                  @click.stop="toggleFavorite(gadget)"
                >
                  <img
                    :src="
                      isFavorite(gadget)
              ...
              <p class="product-price">
                {{ gadget.price.toLocaleString() }} MKD
              </p>
            </article>
          </div>
...
const filteredProducts = computed(() => {
  ...
  return gadgets.filter((gadget) => {
    ...
    const matchesCategory =
      ...
      gadget.category === selectedCategory.value
...
const isFavorite = (gadget) => {
  return favoriteIds.value.includes(
    gadget.id
  )
}

const toggleFavorite = (gadget) => {
  const favorites =
    JSON.parse(
      localStorage.getItem('favorites')
    ) || []

  if (isFavorite(gadget)) {
    const updated =
      favorites.filter(
        (item) =>
          item.id !== gadget.id
